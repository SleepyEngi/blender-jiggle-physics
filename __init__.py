...
def get_reference_rest_pose_matrix(bone):
    rest_bone = get_rest_source_bone(bone)

    # Start from the reference bone's current pose matrix
    matrix = rest_bone.matrix.copy()

    # Remove the reference bone's parent contribution
    if rest_bone.parent:
        matrix = rest_bone.parent.matrix.inverted_safe() @ matrix

    # Re-apply the simulated bone's parent contribution
    if bone.parent:
...
class VirtualParticle:
    ...
    def apply_pose(self):
        ...
        rest_matrix = get_reference_rest_pose_matrix(self.bone)
        loc, rot, scale = rest_matrix.decompose()
